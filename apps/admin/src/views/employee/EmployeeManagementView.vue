      ...
      <section class="modal-dialog" role="dialog" aria-modal="true" :aria-label="`${editingId ? 'Edit' : 'Add'} employee management record`">
        ...
        <section class="address-section">
          ...
          <div class="address-grid">
            <BaseInput v-model="employeeForm.addresses.present.address_line" label="Address Line" />
            <BaseInput v-model="employeeForm.addresses.present.district" label="District" />
            <BaseInput v-model="employeeForm.addresses.present.division" label="Division" />
            <BaseInput v-model="employeeForm.addresses.present.postal_code" label="Postal Code" />
            <BaseInput v-model="employeeForm.addresses.present.country" label="Country" />
          </div>
        </section>
        <section class="address-section">
          <div class="address-heading">
            <h3>Permanent Address</h3>
            <button class="copy-address-btn" type="button" @click="copyPresentAddress">Copy Present Address</button>
          </div>
          <div class="address-grid">
            <BaseInput v-model="employeeForm.addresses.permanent.address_line" label="Address Line" />
            <BaseInput v-model="employeeForm.addresses.permanent.district" label="District" />
            <BaseInput v-model="employeeForm.addresses.permanent.division" label="Division" />
            <BaseInput v-model="employeeForm.addresses.permanent.postal_code" label="Postal Code" />
            <BaseInput v-model="employeeForm.addresses.permanent.country" label="Country" />
          </div>
        </section>
...
function emptyAddress() { return { address_line: "", district: "", division: "", postal_code: "", country: "Bangladesh" }; }
function addressesForForm(addresses = []) {
  const present = addresses.find((item) => item.address_type === "PRESENT") || {};
  const permanent = addresses.find((item) => item.address_type === "PERMANENT") || {};
  return { present: { ...emptyAddress(), ...present }, permanent: { ...emptyAddress(), ...permanent } };
}
function copyPresentAddress() { employeeForm.addresses.permanent = { ...employeeForm.addresses.present }; }
function resetEmployee() { Object.assign(employeeForm, { employee_no: "", first_name: "", first_name_bn: "", last_name: "", last_name_bn: "", employee_type: "STAFF", branch_id: "", department_id: "", designation_id: "", joining_date: "", date_of_birth: "", gender: "", mobile: "", email: "", nid_no: "", blood_group: "", religion: "", photo_url: "", employment_status: "ACTIVE", addresses: addressesForForm() }); }
...
async function openEmployeeForm(row = null) { if (isSuperAdmin && !institutionId.value) return message("error", "Select an institution first."); await loadLookups(); resetEmployee(); editingId.value = row?.employee_id || null; if (row) { try { const response = await employeeApi.getEmployee(row.employee_id); const detail = response.data.data; Object.assign(employeeForm, { ...detail.employee, joining_date: toIsoDate(detail.employee.joining_date), date_of_birth: toIsoDate(detail.employee.date_of_birth), branch_id: detail.employee.branch_id || "", department_id: detail.employee.department_id || "", designation_id: detail.employee.designation_id || "", addresses: addressesForForm(detail.addresses || []) }); } catch (error) { return message("error", error.response?.data?.message || "Failed to load employee details."); } } showForm.value = true; }
...
async function removeEmployee(row) { const name = row.full_name || row.first_name; if (!await confirmAction({ title: "Delete employee?", message: `“${name}” and the related employee record will be permanently deleted.`, confirmText: "Delete employee" })) return; try { await employeeApi.deleteEmployee(row.employee_id); message("success", "Employee deleted successfully."); notify.success("Employee deleted successfully."); await withPreservedScroll(loadEmployees); } catch (error) { const text = error.response?.data?.message || "Failed to delete employee."; message("error", text); notify.error(text); } }
async function removeMaster(row) { const isDepartment = activeTab.value === "departments"; const recordType = isDepartment ? "department" : "designation"; if (!await confirmAction({ title: `Delete ${recordType}?`, message: `This ${recordType} will be permanently deleted.`, confirmText: `Delete ${recordType}` })) return; try { isDepartment ? await employeeApi.deleteDepartment(row.department_id) : await employeeApi.deleteDesignation(row.designation_id); message("success", "Record deleted successfully."); notify.success(`${recordType[0].toUpperCase()}${recordType.slice(1)} deleted successfully.`); await withPreservedScroll(async () => { await loadMaster(); await loadLookups(); }); } catch (error) { const text = error.response?.data?.message || "Failed to delete record."; message("error", text); notify.error(text); } }
