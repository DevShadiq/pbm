...
async function remove(event) { if (!await confirmAction({ title: "Delete event?", message: `“${event.title}” will be permanently deleted.`, confirmText: "Delete event" })) return; try { await eventApi.delete(event.event_id); notify.success("Event deleted successfully."); await withPreservedScroll(load); } catch { message.value = "Failed to delete event."; messageType.value = "error"; notify.error(message.value); } }
onMounted(load);
</script>
